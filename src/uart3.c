...
TaskHandle_t xTaskUart3Notify = NULL;
...
void USART3_IRQHandler()
{
	...
	if (USART3->SR & (USART_FLAG_PE | USART_FLAG_FE | USART_FLAG_ORE ))
	{
		f_rx_err = 1;
	}
	...
	while (USART3->SR & USART_FLAG_RXNE)
	{
		if (rx_len < RX_BUFF_LEN)
			p_uart_rx_curr[rx_len++] = (int8_t)USART3->DR;
		else
			tmp = (int8_t)USART3->DR;
	...
	if ((USART3->SR & USART_FLAG_IDLE))
	{
		...
		vTaskNotifyGiveFromISR(xTaskUart3Notify, &xHigherPriorityTaskWoken);
...
void USART3_Init(void)
{
	...
	//USART_SetDEAssertionTime(USART3, 16);
	//USART_SetDEDeassertionTime(USART3, 16);
...
void DMA_ConfigurationUSART3(void) {
